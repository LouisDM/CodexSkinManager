...
皮肤详情必须明确显示发布者信任状态、素材使用权和导出限制。未经授权的素材只能本机使用；允许公开分发的皮肤也必须保留非商用、同人、无背书和第三方权利边界。
...
同一对象的相反操作应放在一起。`.codexskin` 的导入与导出并排出现在顶部工具栏，不再分散在资料库底部和详情菜单；受授权限制时，导出入口保留位置并解释原因，而不是静默消失。
