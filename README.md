...
一款原生 macOS Codex 皮肤管理器，支持安全导入、预览、切换、重新应用、恢复和导出 `.codexskin` 包。
...
GitHub Release 公开六个可下载皮肤包：孟川两套、哈基米、蔡徐坤、蒂法和柳七月。源码版 `1.1.5` 仍保留旧启动器迁移、导入和流程验证能力。
...
- 原生 SwiftUI 双栏工作区：左侧直接选择皮肤，右侧集中预览、状态和应用操作。
- 主界面保持单窗口；重复打开应用或批量导入 `.codexskin` 会复用同一个工具窗口。
- 窗口顶部把 `.codexskin` 的导入与导出并排放在同一操作组；任意已安装皮肤都可以导出，公开分享前按包内权利说明自行确认素材边界。
- 筛选收进皮肤列表顶部菜单，不再单独占一整列；列表会同时显示皮肤总数、版本和最近使用状态。
...
前往 [GitHub Releases](https://github.com/LouisDM/CodexSkinManager/releases) 下载当前版本。`v1.0.1` 是历史发布，只包含当时已有的两套孟川公开皮肤；`v1.1.0` 包含三套公开皮肤；`v1.1.1` 包含五套公开皮肤；`v1.1.2` 包含六套公开皮肤；`v1.1.3` 开放所有已安装皮肤导出；`v1.1.4` 精简皮肤列表和详情展示；当前 `v1.1.5` 修复重复导入时可能打开多个工具窗口的问题，发布产物应包含六套皮肤：

- `Codex-Skin-Manager-v1.1.5-macOS.zip`：管理器应用，已内置两套孟川皮肤，并支持导入、切换和导出任意已安装皮肤。
...
日常使用时，直接在左侧点选皮肤并在右侧确认预览与权利信息；顶部工具栏可直接导入或导出 `.codexskin`。任何已安装皮肤都可以导出，包内权利说明只用于提醒公开分享边界，不再锁住 App 操作。需要同时比较多套皮肤时，用 `⌥⌘I` 收起详情进入画廊。顶部筛选菜单只保留“全部皮肤”和“最近使用”两个日常视图。
